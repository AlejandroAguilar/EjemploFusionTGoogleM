	...
	google.maps.event.addListener(map, 'click', 
			...
			if(lineas==true){
				...
				var bermudaTriangle = new google.maps.Polygon({
					...
					arrCuadros[numCuadros]=caminito;
					numCuadros=numCuadros+1;
			...
			if(rutas==true){
				...
				alert('Son '+google.maps.geometry.spherical.computeLength(caminito)+' metros');
				...
				var bermudaTriangle = new google.maps.Polygon({
					...
					//Se le agrega un metodo para mostrar el area
					google.maps.event.addListener(bermudaTriangle, 'click', 
									function(event) {  
										    var temporal=bermudaTriangle.getPath();
											alert('Son '+google.maps.geometry.spherical.computeArea(temporal)+' metros cuadrados');
											//alert('Son '+google.maps.geometry.spherical.computeDistanceBetween(temporal[0],temporal[1]));
											//alert('Son '+google.maps.geometry.spherical.computeDistanceBetween(temporal[1],temporal[2]));
											//alert('Hola');
									  }
						);
...
function habilitarRutas() {  
	...
	//Si esta seleccionada la opcion de rutas
     if(formcache.marcaRutas.checked){
    		rutas=true; 
		marcas=false;
		lineas=false;
		formcache.habilitaMarca.checked=false;
		formcache.marcaLineas.checked=false;
	}else{
		rutas=false;
	}
...
function habilitarMarcas() {  
	...
	var formcache=document.targetmain;
    //si se esta habilitando una marca
...
function habilitarLineas() {  
    
	var formcache=document.targetmain;

	
	if(formcache.marcaLineas.checked){
		lineas=true;
    	marcas=false;
		rutas=false; 
		formcache.marcaRutas.checked=false;
...
<table>
<form name='targetmain' action='cargarMapaLineas.php' method='post'>

<p>
<input type='checkbox' name='habilitaMarca'  id='on' onClick='habilitarMarcas()'>Habilitar Marcas</input>
<input type='checkbox' name='marcaRutas'  id='este' onClick='habilitarRutas()'>Trazar Areas</input>
<input type='checkbox' name='marcaLineas'  id='lineas' onClick='habilitarLineas()'>Trazar Lineas</input>


    <input id='datosMarcas' name='datosMarcas' type='hidden' value='' />
	<input id='datosAreas' name='datosAreas' type='hidden' value='' />
	<input id='datosLineas' name='datosLineas' type='hidden' value='' />
